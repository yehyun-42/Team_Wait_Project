

# 주제: 유기동물 보호소 관제 시스템
**Stray Animal Shelter Monitoring System**
<br />

DW아카데미 AIoT 기반 드론 관제 시스템 개발 4회차 3팀 팀프로젝트
<br />

**🥽 문제 인식**
<br />

- 보호소 부지가 넓거나 실외 공간이 많은 경우, 제한된 인력으로 모든 유기 동물의 상태를 실시간 파악 불가.
- 사고 및 탈출 발생 시 보호 중인 유기 동물의 부상이나 민간 지역으로 유실 되는 등의 2차 피해 발생 가능.
- 수작업 및 단순 일지 데이터는 누락 가능성이 높으며 대규모 통계 도출이 어려움.
- 지자체 및 기관 보고 시, 명확한 근거 자료 및 통계의 부족으로 인한 예산 절감 및 운영 개선안 마련이 어려움.
<br />

**🛠️ 기대 효과**
<br />

- 드론을 활용하여 공중에서 사각지대 없이 실시간 순찰을 통하여 장소 및 시간에 따른 위험 상황을 즉각 포착.
- AI를 통해 유기동물을 식별하는 즉시 관제원에게 알림으로서 사고 발생 후 초동 조치까지의 시간 최소화.
- 축종 및 세부 품종 정보를 DB에 자동 적재, 조치 상태를 관리하여 정밀한 이벤트 및 이력 추적.
- 각종 주요 데이터를 행정 보고용 리포트로 활용하여 정확한 지표에 기반한 보호소 운영 및 예산 편성.
<br />

# 👨‍💻 팀원: 5명
팀장: 강동현
<br />
팀원: 서기원, 배가영, 장지웅, 서예현
<br />

# 💻 1차
<br />

진행 기간: 2026.05.26 ~ 2026.06.16 (13일)
<br />

- Frontend Developer
- AI 학습 담당
- ESP32-CAM 연동 담당
- CRUD 및 auth 등, 사용자 접근성 담당
- Database pipeline developer
<br />

**Tech Stack**
<br />

**Languages**

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)

**Database**

![SQLAlchemy](https://img.shields.io/badge/SQLAlchemy-%23D71F00.svg?style=for-the-badge&logo=sqlalchemy&logoColor=white)
![Flask-Migrate](https://img.shields.io/badge/Flask--Migrate-%23000000.svg?style=for-the-badge&logo=flask&logoColor=white)

**Web**

![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white)

**AI**

![YOLO](https://img.shields.io/badge/YOLO-00FFFF?style=for-the-badge&logo=yolo&logoColor=black)

# 💻 2차
<br />

진행 기간: 2026.08.24 ~ 2026.08.25(현재 진행중)
<br />

- PM(아키텍처 설계, 공통 보안 모듈)
- 인프라 엔지니어(AI 및 웹소켓 통신)
- 데이터 엔지니어(DB 최적화, 통계 가공)
- 프론트엔드(화면 및 DB 연동)
- QA 및 DevOps 엔지니어
<br />
